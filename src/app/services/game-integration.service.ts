...
export class GameIntegrationService {
  ...
  processGameResult(gameResult: GameResult): Observable<any> {
    ...
    return this.checkActiveChallenges(currentUser.id).pipe(
  ...
  checkActiveChallenges(userId?: number): Observable<IChallenge[]> {
    ...
    return this.challengeService.getActiveChallengesByUserAndGame(targetUserId, this.ECO_DRAG_DROP_GAME_ID).pipe(
      map((response: any) => {
